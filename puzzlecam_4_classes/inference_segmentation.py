...
if __name__ == '__main__':
    ...
    train_dataset = VOC_Dataset_For_WSSS(args.data_dir, 'train_aug', pred_dir)
    ...
    model.eval()
    eval_timer.tik()
    tensorboard_dir = create_directory(f'./experiments/tensorboards/test/')
    writer = SummaryWriter(tensorboard_dir)
    vis_dir = '/research/axns2/mabdelfa/core/puzzle_zerowaste_4_org/vis/'
    cc = 0
    def visualize(image_id, images, logits, gt=False):
        predictions = torch.argmax(logits, dim=1)
        image = get_numpy_from_tensor(images[0])
        pred_mask = get_numpy_from_tensor(predictions[0])
        ...
        gt_mask = Image.open('/research/axns2/mabdelfa/ZeroWasteAug_voc_format/SegmentationClass/'+image_id+'.png').convert('P')
        gt_mask = asarray(gt_mask)
        gt_mask = gt_mask.astype(np.uint8)
        #print(gt_mask.shape)
        #print(type(gt_mask))
        gt_mask = decode_from_colormap(gt_mask, train_dataset.colors)
        gt_mask = cv2.resize(gt_mask, (w, h), interpolation=cv2.INTER_NEAREST)
        

        pred_image = cv2.addWeighted(image, 0.3, pred_mask, 0.85, 0)[..., ::-1]
        typee = '_gt' if gt else '_pred'
        cv2.imwrite(vis_dir + str(image_id) + typee +'.png', pred_image)

        gt = True
        gt_image = cv2.addWeighted(image, 0.3, gt_mask, 0.85, 0)[..., ::-1]
        typee = '_gt' if gt else '_pred'
        cv2.imwrite(vis_dir + str(image_id) + typee +'.png', gt_image)
        #print(vis_dir + str(image_id))
        
        image = image.astype(np.float32) / 255.

    def inference(image_id, images, gt_mask, image_size, scale):
        images = images.cuda()
        logits = model(images)

        if scale == 1:
            visualize(image_id, images, logits, False)
            #visualize(image_id, images, gt_mask, True)
    ...
    with torch.no_grad():
        ...
        for step, (ori_image, image_id, gt_mask) in enumerate(dataset):
            ...
            for scale in scales:
                ...
                cams = inference(image_id, images, gt_mask, (ori_h, ori_w), scale)
