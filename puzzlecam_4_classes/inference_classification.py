...
if __name__ == '__main__':
    ...
    def visualize(image_id, images, obj_cams, b):
        ...
        cv2.imwrite(vis_dir + str(image_id) + typee +'.png', image)
    ...
    valid_loader_for_seg = DataLoader(valid_dataset_for_seg, batch_size=8, num_workers=1, drop_last=True)

    def get_cam(ori_image, scale, image_id, label):
        # preprocessing
        ...
        if scale == 1:
            with torch.no_grad():
                for step, (images, labels, gt_masks) in enumerate(valid_loader_for_seg):
                    images = images.cuda()
                    labels = labels.cuda()
                    _, features = model(images, with_cam=True)
                    ...
                    print('creating cam...', images.shape, cams.shape)
                    obj_cams = cams.max(dim=1)[0]
                    for b in range(8):
                        visualize(cc, images, obj_cams, b)
                        cc+=1
                quit() """
    ...
    with torch.no_grad():
        ...
        for step, (ori_image, image_id, label, gt_mask) in enumerate(dataset):
            ...
            cams_list = [get_cam(ori_image, scale, image_id, label) for scale in scales]
